...
import { Plus, Pencil, Trash2, CalendarDays, FileText, Copy } from "lucide-react";
...
function JadwalTab({ isAdmin }: { isAdmin?: boolean }) {
  ...
  const byStatus = { ongoing: schedules?.filter(s => s.status === "ongoing") ?? [], planned: schedules?.filter(s => s.status === "planned") ?? [], done: schedules?.filter(s => s.status === "done") ?? [] };

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-semibold text-gray-700">Jadwal Program Kerja</h2>
          <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">{byStatus.ongoing.length} berjalan</span>
        </div>
        {isAdmin && (
      ...
      {isLoading ? (
        <div className="animate-pulse space-y-3">{[1, 2, 3].map(i => <div key={i} className="bg-gray-100 rounded-xl h-24" />)}</div>
      ) : !schedules?.length ? (
        <div className="text-center py-12 text-gray-400 text-sm">Belum ada program kerja terjadwal.</div>
      ) : (
        <div className="space-y-3">
          {[...byStatus.ongoing, ...byStatus.planned, ...byStatus.done].map(s => (
            <div key={s.id} className={cn("glass-card p-4 group transition-all hover:-translate-y-0.5", s.status === "done" && "opacity-60")}>
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-0.5">PJ: <span className="font-medium">{s.leader}</span></p>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="glass-panel border-white/50 max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editId ? "Edit Program" : "Tambah Program Kerja"}</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Anggota Tim</label>
...
function TemplatesTab({ isAdmin }: { isAdmin?: boolean }) {
  ...
  const catColor: Record<string, string> = {
    laporan: "bg-rose-100 text-rose-700 border-rose-200",
    administrasi: "bg-sky-100 text-sky-700 border-sky-200",
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        <div className="animate-pulse grid gap-3 sm:grid-cols-2">{[1,2,3].map(i => <div key={i} className="bg-gray-100 rounded-xl h-20" />)}</div>
      ) : !templates?.length ? (
        <div className="text-center py-12 text-gray-400 text-sm">Belum ada template.</div>
      ) : (
            ...
            <div key={t.id} className="glass-card p-4 group transition-all hover:-translate-y-0.5 cursor-pointer" onClick={() => setViewing(t)}>
              <div className="flex justify-between items-start gap-2">
                <div className="flex-1 min-w-0">
                  <Badge className={cn("text-xs border mb-2", catColor[t.category] ?? catColor.lainnya)}>{catLabel[t.category] ?? t.category}</Badge>
      ...
      <Dialog open={!!viewing} onOpenChange={() => setViewing(null)}>
        <DialogContent className="glass-panel border-white/50 max-w-2xl max-h-[80vh]">
          ...
          <pre className="mt-2 text-sm text-gray-700 whitespace-pre-wrap overflow-y-auto max-h-[50vh] bg-white/40 rounded-xl p-4 font-sans">{viewing?.content}</pre>
        </DialogContent>
      </Dialog>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Jadwal program kerja KKN dan template dokumen</p>
      </div>
